...
const dynamicSkills = ref<{ name: string }[] | null>(null);
    ...
    if (res.ok) {
      ...
      if (settings.skills) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          dynamicSkills.value = parsed.map((s: string) => ({ name: s }));
        }
      }
    }
  } catch {
...
const services = computed(() => {
  return dynamicSkills.value || FALLBACK_SERVICES;
});
</script>
