...
const init = (_canvas: HTMLCanvasElement | null) => {
  ...
  // Handle WebGL context restoration after tab switch
...
const setIsActive = (value: boolean) => {
  isActive = value;
  // Force visibility check on next frame
  if (value && canvas) {
    visible = false; // Reset so tick() re-evaluates
  }
};

const handleVisibilityChange = () => {
  if (document.visibilityState === "visible" && instance && isActive) {
    // Force a render on tab return
    visible = false;
  }
};
